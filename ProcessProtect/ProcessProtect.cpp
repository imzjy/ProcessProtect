...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING) {
	...
	OB_OPERATION_REGISTRATION operations[] = {
		...
		OnPreOpenProcess, nullptr //pre, post operations
	};

	OB_CALLBACK_REGISTRATION reg = {
		OB_FLT_REGISTRATION_VERSION,
		1,  //operation count, we only care about the OpenProcess system call
		RTL_CONSTANT_STRING(L"12345.6072"),
		nullptr, //context
		operations
	};

	auto status = STATUS_SUCCESS;
	UNICODE_STRING deviceName = RTL_CONSTANT_STRING(L"\\Devices\\" PROCESS_PROTECT_NAME);
...
NTSTATUS ProcessProtectDeviceControl(PDEVICE_OBJECT, PIRP Irp) {
	...
	{
	case IOCTL_PROCESS_PROTECT_BY_ID:
	{
	...
	case IOCTL_PROCESS_UNPROTECT_BY_ID:
	{
		auto size = stack->Parameters.DeviceIoControl.InputBufferLength;
		if (size % sizeof(ULONG) != 0)
		{
